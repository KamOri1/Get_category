...
class CollectCategory:
    COUNTRY_SLAG = ['at', 'co.uk', 'de', 'pl', 'fr', 'es', 'hu','it', 'se', 'pt', 'dk', 'cz', 'fi', 'nl', 'no', 'sk']
...
def zadanie_a():
    ...
    a = CollectCategory(cat_all, Cat_1='/outdoor-furniture/lounge-sets/',).saveAllCategory()
...
if __name__ == '__main__':
    with Pool(processes=4) as pool:
        start = time.time()
        p1 = pool.apply_async(zadanie_a)
        p2 =pool.apply_async(zadanie_b)
        p3 =pool.apply_async(zadanie_c)
        p4 =pool.apply_async(zadanie_d)
        pool.close()
        pool.join()
        end = time.time()
